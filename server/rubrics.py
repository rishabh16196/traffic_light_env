...
def _linear_score(value: float, perfect: float, fail: float) -> float:
    ...
    Values better than perfect clamp to 1.0; worse than fail clamp to 0.0.
...
TASK_THRESHOLDS: Dict[str, Dict[str, Tuple[float, float]]] = {
    "balanced": {
        "avg_waiting": (4.0, 20.0),
        "throughput": (180.0, 60.0),
    },
    "rush_hour_ns": {
        "avg_waiting": (8.0, 25.0),
        "throughput": (200.0, 80.0),
    },
    "rush_hour_ew": {
        "avg_waiting": (8.0, 25.0),
        "throughput": (200.0, 80.0),
    ...
    "alternating_surge": {
        "avg_waiting": (10.0, 30.0),
        "throughput": (180.0, 60.0),
    },
    "random_spikes": {
        "avg_waiting": (10.0, 30.0),
        "throughput": (160.0, 50.0),
    },
    "gridlock": {
        "avg_waiting": (15.0, 40.0),
        "throughput": (250.0, 100.0),
    },
    "emergency_vehicle": {
        "avg_waiting": (4.0, 20.0),
        "throughput": (180.0, 60.0),
...
EMERGENCY_CLEARANCE_THRESHOLDS: List[Tuple[int, float]] = [
    (3, 1.0),    # Cleared within 3 steps: perfect
    (8, 0.7),    # Cleared within 8 steps: good
    (15, 0.4),   # Cleared within 15 steps: passable
    (30, 0.1),   # Cleared within 30 steps: poor
]
...
class TrafficLightRubric(TrajectoryRubric):
    ...
    The final score and per-component breakdown are stored in
    ``observation.metadata["grade"]`` on the terminal step.
    """
    ...
    def _grade_emergency(self, trajectory: List[Tuple[Any, Any]]) -> float:
        ...
        # Find when emergency was cleared by checking when emergency_lane goes to -1
        # after having been >= 0
        ...
        for _, obs in trajectory:
            e_lane = getattr(obs, "emergency_lane", -1)
